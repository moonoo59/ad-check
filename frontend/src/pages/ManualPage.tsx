 ...
 *
 * 권한 체계 반영:
 * - can_copy 권한이 있는 사용자에게는 파일 전송 관련 섹션 표시
 * - can_view_stats 권한이 있는 사용자에게는 통계 조회 관련 섹션 표시
 *
 * useAuth()로 현재 사용자 역할과 권한을 확인해 렌더링을 분기한다.
 */
  ...
  return (
    <>
      ...
      <InfoBox type="warning">
        초기 비밀번호는 관리자에게 문의하세요. 로그인 후 우측 상단 → <strong>[비밀번호]</strong> 버튼에서 즉시 변경하세요.
      </InfoBox>
      ...
      <Table
        ...
        rows={[
          ...
          ['통계', '요청 건수 통계 및 CSV 내보내기.', '통계 조회 권한 필요'],
          ['관리자 메뉴', '채널 매핑, 사용자 관리, 감사 로그.', '관리자 전용'],
        ]}
      />
      <InfoBox type="info">
        메뉴에 <strong>[통계]</strong>가 보이지 않는다면 통계 조회 권한이 없는 상태입니다. 관리자에게 권한 부여를 요청하세요.
      </InfoBox>
      ...
      <Table
        ...
        rows={[
          ...
          ['비밀번호를 잊어버렸어요.', '가입 시 연락처를 등록했다면 로그인 화면의 [비밀번호 초기화]를 사용하세요. 연락처가 없으면 관리자에게 초기화를 요청하세요.'],
          ['통계 메뉴가 보이지 않아요.', '통계 조회 권한이 없는 상태입니다. 관리자에게 권한 부여를 요청하세요.'],
          ['파일 탐색/승인 버튼이 없어요.', '파일 전송 권한이 없는 상태입니다. 관리자에게 권한 부여를 요청하세요.'],
        ]}
      />
...
ls /Volumes/data    # Logger Storage 마운트 확인`}</CodeBlock>
      ...
      <p className="text-sm text-gray-700 mb-2">
        상단 <strong>[관리자 메뉴] → [사용자 관리]</strong>에서 계정과 기능 권한을 관리합니다.
      </p>
      <Table
        headers={['기능', '방법']}
        rows={[
          ['계정 생성',       '[+ 사용자 추가] 버튼 → 계정명/표시명/역할/권한/초기 비밀번호 입력'],
          ['역할 변경',       '목록에서 [수정] 클릭 → 역할 변경 → 저장'],
          ['권한 부여/해제',  '목록에서 [수정] 클릭 → 파일 전송 / 통계 조회 체크박스 조정 → 저장'],
          ['계정 비활성화',   '목록에서 [비활성화] 클릭 → 확인 (계정 삭제 불가)'],
          ['비밀번호 초기화', '목록에서 [비밀번호 초기화] 클릭 → 임시 비밀번호 입력'],
        ]}
      />

      <SubTitle>역할 및 기능 권한 체계</SubTitle>
      <Table
        headers={['구분', '설명']}
        rows={[
          ['역할: admin (시스템 관리자)', '모든 기능 접근. 채널 매핑, 사용자 관리, 감사 로그, 통계, 파일 전송 포함.'],
          ['역할: tech_team (대표 담당자)', '기본적으로 파일 전송 권한 보유. 관리자 메뉴 접근 불가.'],
          ['역할: ad_team (채널 담당자)', '기본적으로 요청 등록/조회만 가능. 관리자 메뉴 접근 불가.'],
        ]}
      />
      <Table
        headers={['기능 권한', '기능 범위', '기본 부여 역할']}
        rows={[
          ['파일 전송', '파일 탐색 시작, 파일 선택, 승인/반려, 복사 실행, 파일 삭제, 오전송 수정', 'admin, tech_team (ad_team도 권한 부여 시 가능)'],
          ['통계 조회', '통계 대시보드 조회, CSV 내보내기', 'admin'],
        ]}
      />
      <InfoBox type="tip">
        역할과 무관하게 어떤 사용자에게든 개별적으로 기능 권한을 부여할 수 있습니다.
        예: 채널 담당자에게 파일 전송 권한을 부여하면 본인 담당 채널 요청 등록뿐 아니라 탐색·승인·복사까지 직접 처리할 수 있습니다.
      </InfoBox>
      <InfoBox type="info">
        <strong>admin 역할</strong>은 권한 설정과 무관하게 항상 모든 기능을 사용할 수 있습니다.
        사용자 관리 테이블에서 admin의 권한 열은 "자동"으로 표시됩니다.
      </InfoBox>
      ...
      <Table
        ...
        rows={[
          ...
          ['감사 로그', '관리자 메뉴 → 감사 로그', '전체 사용자 작업 이력 (로그인/요청/승인/반려 등) 날짜·사용자별 필터'],
        ]}
      />
...
# 기존 앱을 덮어쓰거나 먼저 삭제 후 이동`}</CodeBlock>
      ...
      <Table
        ...
        rows={[
          ...
          ['사용자 관리', '관리자 메뉴 → 사용자 관리', '계정 생성/역할 변경/권한 부여/비밀번호 초기화'],
  ...
  return (
    <div className="app-page app-page--narrow">
      ...
      {/* 렌더링 분기:
          ...
          - admin + 사용자 탭 → UserManual (미리보기, 모든 권한 표시)
          - 일반 사용자      → UserManual (본인 역할/권한에 맞는 섹션만 표시)
      */}
      {isAdmin && viewMode === 'admin' ? (
        ...
        <UserManual
          ...
          canCopy={isAdmin || !!user?.can_copy}
          canViewStats={isAdmin || !!user?.can_view_stats}
          urls={urls}
        />
      )}
      </div>
