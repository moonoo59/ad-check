...
import { env } from './env';
import { SQLITE_UTC_NOW_EXPRESSION, utcNow } from '../common/datetime';
...
function initMigrationTable(): void {
  db.exec(`
    CREATE TABLE IF NOT EXISTS schema_migrations (
      ...
      applied_at TEXT NOT NULL DEFAULT (${SQLITE_UTC_NOW_EXPRESSION})
    )
  `);
}
...
function runMigrations(): void {
  ...
  for (const filename of sqlFiles) {
    ...
    // SQLite 테이블 재생성 마이그레이션(DROP TABLE + CREATE TABLE 패턴)에서
    // FK 제약이 활성화된 채로 DROP을 시도하면 child 테이블이 있을 때 실패한다.
    // PRAGMA foreign_keys 는 트랜잭션 외부에서만 변경 가능하므로
    // migrate() 호출 전/후에 일시적으로 비활성화 후 재활성화한다.
    const migrate = db.transaction(() => {
      db.exec(sql);
      db.prepare('INSERT INTO schema_migrations (filename, applied_at) VALUES (?, ?)').run(filename, utcNow());
    });

    try {
      db.pragma('foreign_keys = OFF'); // 테이블 재생성 패턴을 위해 일시 비활성화
      migrate();
      db.pragma('foreign_keys = ON');  // 완료 후 즉시 재활성화
      console.log(`[DB] 마이그레이션 적용 완료: ${filename}`);
      appliedCount++;
    } catch (err) {
      db.pragma('foreign_keys = ON');  // 실패해도 반드시 재활성화
...
function assertForeignKeyIntegrity(): void {
  const violations = db.pragma('foreign_key_check', { simple: false }) as Record<string, unknown>[];

  if (violations.length === 0) {
    return;
  }

  console.error('[DB] foreign_key_check 실패:', violations);
...
function seedDefaultChannelMappings(): void {
  const defaultMappings = [
    { storage_folder: 'CNBC', display_name: '비즈', nas_folder: '비즈', description: 'CNBC 비즈니스 채널' },
    { storage_folder: 'ESPN', display_name: '스포츠', nas_folder: '스포츠', description: 'ESPN 스포츠 채널' },
    { storage_folder: 'ETV', display_name: '라이프', nas_folder: '라이프', description: 'ETV 라이프 채널' },
    { storage_folder: 'FIL', display_name: '퍼니', nas_folder: '퍼니', description: 'FIL 퍼니 채널' },
    { storage_folder: 'GOLF', display_name: '골프', nas_folder: '골프', description: 'GOLF 골프 채널' },
    { storage_folder: 'NICK', display_name: '골프2', nas_folder: '골프2', description: 'NICK 골프2 채널' },
    { storage_folder: 'PLUS', display_name: '플러스', nas_folder: '플러스', description: 'PLUS 플러스 채널' },
  ];

  const now = utcNow();
  const insert = db.prepare(`
    INSERT OR IGNORE INTO channel_mappings
      (storage_folder, display_name, nas_folder, description, is_active, created_at, updated_at)
    VALUES
      (@storage_folder, @display_name, @nas_folder, @description, 1, @now, @now)
  `);

  let insertedCount = 0;
  for (const mapping of defaultMappings) {
    const result = insert.run({ ...mapping, now });
    insertedCount += Number(result.changes ?? 0);
  }

  if (insertedCount > 0) {
    console.log(`[DB] 기본 채널 매핑 ${insertedCount}건 보강 완료.`);
  }
}
