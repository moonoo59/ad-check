 ...
 * 출력 대상:
 *   1. 터미널 콘솔 (레벨별 색상)
 *   2. ~/Library/Application Support/AdCheck/logs/app-YYYY-MM-DD.log (기본값)
 *
 * 사용법:
...
// 로그 파일 디렉토리
// - 기본: ~/Library/Application Support/AdCheck/logs
// - APP_SUPPORT_DIR 또는 LOGS_PATH로 오버라이드 가능
const DEFAULT_APP_SUPPORT_DIR = process.platform === 'darwin'
  ? path.join(os.homedir(), 'Library', 'Application Support', 'AdCheck')
  : path.join(process.cwd(), '.runtime', 'AdCheck');
const LOG_DIR = process.env.LOGS_PATH
  ?? path.join(process.env.APP_SUPPORT_DIR ?? DEFAULT_APP_SUPPORT_DIR, 'logs');
...
function writeToFile(line: string): void {
  ...
  if (!stream) {
    ...
    fileStreams.set(logFile, stream);
  }
