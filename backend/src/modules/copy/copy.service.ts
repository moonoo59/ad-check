 ...
 * 파일 복사 서비스
 *
 * 승인된 요청의 파일을 Logger Storage → 서버 로컬 전달 스토리지로 복사한다.
...
export async function executeCopyJobs(requestId: number, approvedByUserId: number): Promise<void> {
  ...
  // 로컬 전달 경로 존재 확인 (없으면 index.ts가 기동 시 생성하지만, 혹시 삭제된 경우 대비)
  if (!fsSync.existsSync(env.LOCAL_DELIVERY_PATH)) {
    log.error(`로컬 전달 경로 없음`, { path: env.LOCAL_DELIVERY_PATH });
    markCopyFailed(requestId, '서버 로컬 전달 경로가 존재하지 않습니다. 관리자에게 문의하세요.', approvedByUserId, wasEditing);
    return;
  }
  ...
  const items = db.prepare(`
    ...
    WHERE ri.request_id = ? AND ri.item_status IN ('approved', 'failed')
    -- 'failed' 포함: prepareForRetryCopy()가 approved로 초기화하지만,
    -- 부분 실패 등으로 failed 상태가 남아 있을 경우에도 재복사 대상이 되도록 안전망 확보
  ...
  if (failCount === 0) {
    ...
    db.prepare(`
      ...
      VALUES (?, 'copy_done', 'requests', ?, ?)
    `).run(approvedByUserId, requestId, completedNow);
    ...
    db.prepare(`
      ...
      VALUES (?, 'copy_failed', 'requests', ?, ?, ?)
    `).run(approvedByUserId, requestId, `${failCount}개 항목 복사 실패`, completedNow);
...
async function copySingleFile(
  ...
  // 복사 대상 경로 구성
  // dest: {LOCAL_DELIVERY_PATH}/{요청일자 YYYY-MM-DD}/{nas_folder}/{파일명}
  // 요청일자 기준으로 상위 폴더 구성 (방송 송출일 대신 요청 생성일 사용)
  // DB는 UTC로 저장되므로 KST(+09:00) 기준 날짜로 변환
  const requestDate = utcToKstDate(item.request_created_at);
  const sourcePath = resolvePathWithinRoot(env.LOGGER_STORAGE_MOUNT, item.file_path, '복사 원본 경로');
  const destDir = joinPathWithinRoot(env.LOCAL_DELIVERY_PATH, [requestDate, item.nas_folder], '로컬 전달 대상 폴더');
  const destPath = joinPathWithinRoot(env.LOCAL_DELIVERY_PATH, [requestDate, item.nas_folder, item.file_name], '로컬 전달 대상 파일');
  const now = utcNow();
  ...
  db.prepare(`
    INSERT INTO audit_logs (user_id, action, entity_type, entity_id, created_at)
    VALUES (?, 'copy_start', 'copy_jobs', ?, ?)
  `).run(approvedByUserId, jobId, startedAt);
  ...
  try {
    ...
    await copyFileWithProgress(sourcePath, destPath, jobId, totalBytes);
    ...
    db.prepare(`
      INSERT INTO audit_logs (user_id, action, entity_type, entity_id, created_at)
      VALUES (?, 'copy_done', 'copy_jobs', ?, ?)
    `).run(approvedByUserId, jobId, completedAt);

    log.info(`항목 ${item.item_id} 로컬 복사 완료`, {
      dest: destPath,
      sizeMB: fileSizeMB,
      elapsedSec,
    });
    ...
    // 복사 실패 시 부분 복사된 파일 정리 (재시도 시 덮어쓰기 오류/디스크 낭비 방지)
    try {
      if (fsSync.existsSync(destPath)) {
        await fs.unlink(destPath);
        log.info(`항목 ${item.item_id}: 복사 실패로 부분 파일 삭제`, { destPath });
      }
    } catch (cleanupErr) {
      log.warn(`항목 ${item.item_id}: 부분 파일 삭제 실패 (무시)`, {
        destPath,
    ...
    db.transaction(() => {
      // progress_bytes = 0: 실패한 항목 진행률을 0으로 초기화 (75% 완료처럼 보이는 오표시 방지)
      db.prepare(`
        UPDATE copy_jobs
        SET status = 'failed', error_message = ?, progress_bytes = 0, updated_at = ?
        WHERE id = ?
    ...
    db.prepare(`
      INSERT INTO audit_logs (user_id, action, entity_type, entity_id, detail, created_at)
      VALUES (?, 'copy_failed', 'copy_jobs', ?, ?, ?)
    `).run(approvedByUserId, jobId, errorMessage, failedAt);
 ...
 */
function markCopyFailed(requestId: number, reason: string, userId: number, keepEditing = false): void {
  const now = utcNow();
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      INSERT INTO audit_logs (user_id, action, entity_type, entity_id, detail, created_at)
      VALUES (?, 'copy_failed', 'requests', ?, ?, ?)
    `).run(userId, requestId, reason, now);
  })();
}
