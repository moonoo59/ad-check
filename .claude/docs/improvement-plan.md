...
> 현재 역할: 2026-03-23 개선 작업의 구현 기록 + 후속 고도화 설계 링크
> 주의: 현재 오픈 버그 우선순위 기준 문서는 아님
> 우선 참조: `./current-priority-map-20260324.md`, `./comprehensive-audit-roadmap-20260324.md`
...
- 광고 구간 자동 컷팅/복사 기능은 별도 설계서에서 관리한다.
- 문서: `./clip-extraction-design.md`
- 구현 계획 및 관리자 ON/OFF 전략 문서: `./clip-extraction-implementation-plan.md`
- 핵심 방향:
  - `monitoring_time`을 `HH:MM:SS`로 표준화
  - 광고 길이(`15/30/45초 등`)와 앞뒤 버퍼 저장
  - `clip_jobs` 기반 구간 추출 후 `copy_jobs`로 NAS 전달
  - 기존 전체 파일 복사(`full_file`)와 공존 가능하게 설계
