...
> 참고: 광고 구간 자동 컷팅/복사 설계는 `./clip-extraction-design.md`를 따른다.
...
> 주의: 이 섹션은 현재 “원본 파일 전체 복사” 기준이다.
> 향후 `clip_with_buffer` 모드에서는 선택된 원본 파일에서 광고 구간을 먼저 추출한 뒤 NAS로 전달하며,
> 상세 설계는 `./clip-extraction-design.md`를 따른다.
