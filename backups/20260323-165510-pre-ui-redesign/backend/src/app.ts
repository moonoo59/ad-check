 ...
 * 6. 라우터 (API)
 * 7. 정적 파일 서빙 (Electron 환경: frontend/dist)
 * 8. SPA fallback → index.html (Electron 환경)
 * 9. 404 처리 (API 경로 한정)
 * 10. 전역 에러 핸들러
 *
...
// CORS 설정 - 내부망 전용이지만 프론트와의 포트 차이로 필요
...
// ===== Electron 전용: React 정적 파일 서빙 =====
// IS_ELECTRON=true 이고 FRONTEND_DIST_PATH 가 주입된 경우에만 활성화.
// Vite 빌드 결과물(frontend/dist)을 Express가 직접 서빙한다.
// /api/* 경로는 위의 라우터가 먼저 처리하므로 충돌 없음.
if (env.IS_ELECTRON && env.FRONTEND_DIST_PATH) {
...
// Electron 환경에서는 정적 파일 fallback 이 먼저 처리하므로 API 404 전용
