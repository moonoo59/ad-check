...
router.get('/summary', (req: Request, res: Response): void => {
  ...
  if (!from && !to) {
    ...
    const thisMonthStart = new Date(`${kstNow.getUTCFullYear()}-${String(kstNow.getUTCMonth() + 1).padStart(2, '0')}-01T00:00:00+09:00`);
...
router.get('/monthly', (req: Request, res: Response): void => {
  ...
  const year = parseInt((req.query.year as string) ?? String(kstNow.getUTCFullYear()), 10);
...
router.get('/daily', (req: Request, res: Response): void => {
  const kstNow = new Date(Date.now() + 9 * 60 * 60 * 1000);
  const year  = parseInt((req.query.year  as string) ?? String(kstNow.getUTCFullYear()), 10);
  const month = parseInt((req.query.month as string) ?? String(kstNow.getUTCMonth() + 1), 10);
