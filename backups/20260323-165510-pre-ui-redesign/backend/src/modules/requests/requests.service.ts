...
export function createRequest(dto: CreateRequestDto, requesterId: number): number {
  ...
  const requestId = db.transaction(() => {
    ...
    db.prepare(`
      ...
      VALUES (?, 'request_create', 'requests', ?, datetime('now', 'localtime'))
    `).run(requesterId, reqId);
...
export function getRequests(
  ...
  if (filter.from) {
    ...
    params.push(new Date(`${filter.from}T00:00:00+09:00`).toISOString());
  }

  if (filter.to) {
    conditions.push('r.created_at <= ?');
    params.push(new Date(`${filter.to}T23:59:59+09:00`).toISOString());
  }
...
export function getRequestDetail(
  ...
  const itemsWithDetails = items.map((item) => {
    const fileResults = db.prepare(`
      SELECT * FROM file_search_results
      WHERE request_item_id = ?
      ORDER BY match_score DESC, created_at ASC
    `).all(item.id) as FileSearchResultRow[];
    ...
    const copyJob = db.prepare(`
      ...
      LIMIT 1
    `).get(item.id) as CopyJobRow | undefined ?? null;

    return { ...item, file_search_results: fileResults, copy_job: copyJob };
  });
...
export function selectFile(
  ...
  if (!['tech_team', 'admin'].includes(currentUserRole)) {
    return false;
  }
...
export function rejectRequest(
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      INSERT INTO audit_logs (user_id, action, entity_type, entity_id, created_at)
      VALUES (?, 'request_reject', 'requests', ?, datetime('now', 'localtime'))
 ...
 */
export function prepareForRetryCopy(requestId: number): true | string {
  const request = db.prepare(`
    SELECT id, status FROM requests WHERE id = ? AND is_deleted = 0
  `).get(requestId) as { id: number; status: string } | undefined;
  ...
  if (request.status === 'failed') {
    ...
    db.transaction(() => {
      ...
      db.prepare(`
        ...
        VALUES (NULL, 'copy_retry', 'requests', ?, ?, datetime('now', 'localtime'))
      `).run(requestId, '복사 재시도 준비');
    })();
...
export function approveRequest(requestId: number, reviewerId: number): boolean {
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      INSERT INTO audit_logs (user_id, action, entity_type, entity_id, created_at)
      VALUES (?, 'request_approve', 'requests', ?, datetime('now', 'localtime'))
    `).run(reviewerId, requestId);
  })();
...
export function prepareForResend(
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      INSERT INTO audit_logs (user_id, action, entity_type, entity_id, detail, created_at)
      VALUES (?, 'request_resend', 'requests', ?, ?, datetime('now', 'localtime'))
    `).run(userId, requestId, `재전송 사유: ${reason.trim()}`);
  })();
...
export async function updateRequestItemForCorrection(
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      INSERT INTO audit_logs (user_id, action, entity_type, entity_id, detail, created_at)
      VALUES (?, 'request_item_correct', 'request_items', ?, ?, datetime('now', 'localtime'))
    `).run(
      userId,
      itemId,
      JSON.stringify({
...
export function deleteRequest(requestId: number, adminId: number): true | string {
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      INSERT INTO audit_logs (user_id, action, entity_type, entity_id, created_at)
      VALUES (?, 'request_delete', 'requests', ?, datetime('now', 'localtime'))
